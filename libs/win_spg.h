...
typedef struct spg {
        ...
        //unsigned int *pixels; //data;     // the pixels in the window
        bool keyboard[256]; // = {0};
        bool has_focus;
        MSG message; // = { 0 };     
} spg; 
...
LRESULT CALLBACK WindowProcessMessage(HWND window_handle, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch(message) {
		...
		case WM_PAINT: {
			PAINTSTRUCT paint; //static PAINTSTRUCT paint;
			HDC device_context; //static HDC device_context;
		...
		case WM_MOUSEMOVE: { POINT pos;
			...
			mywin->mouse.x = LOWORD(lParam);//GET_X_LPARAM(lParam);//(int)LOWORD(lParam);
...
unsigned int get_pixel(spg_frame *mywin,int x,int y, unsigned int argb_color){
        return mywin->pixels[mywin->w*hflip(mywin->h,y)+x];
}
void set_pixel(spg_frame *mywin,int x,int y, unsigned int argb_color){
...
void spg_blit(spg_frame *mywin,spg_frame *frame, int x, int y){
...
void spg_open_plugin_window(spg *mywin,void *ptr,int w, int h){
	...
	window_class.lpfnWndProc = WindowProcessMessage;
        ...
        window_class.hCursor = LoadCursor(NULL, IDC_ARROW); // IDC_WAIT IDC_ARROW
	window_class.lpszClassName = (PCSTR)window_class_name;
        window_class.hbrBackground = (HBRUSH) GetStockObject (WHITE_BRUSH); // 
        window_class.lpszMenuName = NULL; // 
	RegisterClass(&window_class);

        // Detta nedan behövs göras innan fönstret öppnas!
	...
	mywin->bitmap_device_context = CreateCompatibleDC(0);
        ...
        //mywin->window_handle = CreateWindow((PCSTR)window_class_name, "BitBlt Test Program ", WS_CHILD | WS_OVERLAPPEDWINDOW | WS_VISIBLE , CW_USEDEFAULT, CW_USEDEFAULT, w, h, (HWND)ptr, NULL, NULL, NULL);
...
void spg_update_window(struct spg *mywin){
